...
- **自动备份**：支持定时自动备份和手动备份，确保数据安全
- **康复评估**：支持入院/出院评估，包括Barthel指数、Brunnstrom分期等专业评估
- **数据统计**：仪表盘实时显示在院患者、今日记录、治疗项目等统计信息
...
3. **康复评估**：
   - 入院评估（Barthel指数、Brunnstrom分期、平衡功能、肌力评定、MMSE、吞咽功能、语言功能）
   - 出院评估（康复效果、家庭指导）
   - 评估报告查看与修改
...
系统提供**自动备份**和**手动备份**两种方式：

**自动备份**（推荐）：
- 系统每日凌晨2点自动执行备份
- 备份内容包括：数据库、配置文件、签名图片
- 备份文件保留7天（可配置）
- 备份位置：`backend/backups/`目录

**手动备份**：
1. 登录Web后台
2. 进入"备份管理"页面
3. 点击"立即备份"按钮
4. 可选择备份内容：数据库/配置文件/签名图片

**手动备份命令**（可选）：
```bash
# 备份数据库（复制database.db文件）
cp backend/data/database.db backups/database/database_$(date +%Y%m%d).db

# 备份配置文件
cp backend/.env backups/config/env_$(date +%Y%m%d)

# 备份照片文件夹
cp -r backend/uploads/photos backups/photos/
```
...
A: 系统默认在每日凌晨2点自动执行备份，可在 `.env` 文件中修改 `BACKUP_AUTO_TIME` 配置。

### Q: 如何恢复备份数据？

A: 登录Web后台 → 备份管理 → 选择备份记录 → 点击"恢复"按钮 → 选择要恢复的内容（数据库/配置文件/签名图片）。

### Q: 康复评估包含哪些内容？

A: 入院评估包括：Barthel指数、Brunnstrom分期、平衡功能、肌力评定、MMSE认知功能、吞咽功能、语言功能等；出院评估包括康复效果和家庭指导。

### Q: 如何查看待完成评估的患者？

A: 在Web后台仪表盘会显示待完成评估的患者列表，也可在患者列表中看到哪些患者缺少入院或出院评估。
